    ...
    >
      ...
      {zones &&
        ...
        zones.map((zone, index) => {
          if (!Array.isArray(zone.coordsArray) || zone.coordsArray.length === 0)
            return null;

          const randomColor = getRandomColor();
          const positions = zone.coordsArray.map((coord) => [
            coord.lat,
            coord.lng,
          ]);
          ...
          return (
            <Polygon key={index} positions={positions} color={randomColor}>
              <Popup>
                {zone.name} / {zone.technician?.firstname}{" "}
                {zone.technician?.lastname}
              </Popup>
            </Polygon>
          );
        })}
