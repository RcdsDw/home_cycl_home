...
import {
  ...
  deleteInterventionProduct,
  updateInterventionProduct,
} from "../actions/interventionProduct";
...
export default function SelectProducts({
  ...
  const handleProductChange = async (selectedIds) => {
    ...
    const addedProductIds = selectedIds.filter(
    ...
    const newSelectedProducts = selectedIds.map((productId) => {
      const existingItem = selectedProducts?.find(
        (item) => parseID(item.product) === productId,
      );
  ...
  const handleProductCreateWithData = async (
    productId,
    currentSelectedProducts,
  ) => {
    const productData = products.find((p) => parseID(p) === productId);

    const productToCreate = currentSelectedProducts.find(
      (item) => parseID(item.product) === productId,
    );
    ...
    try {
      const createdItem = await createInterventionProduct(
        newInterventionProduct,
      );
      ...
      console.error(
        `Erreur lors de la création du produit d'intervention ${productId}:`,
        error,
      );

      const rollbackProducts = currentSelectedProducts.filter(
        (item) => parseID(item.product) !== productId,
      );
  ...
  const handleProductRemove = async (productId) => {
    ...
    if (productToRemove.id || productToRemove["@id"]) {
      try {
        ...
        console.error(
          `Erreur lors de la suppression du produit d'intervention ${productId}:`,
          error,
        );
        return;
      }
