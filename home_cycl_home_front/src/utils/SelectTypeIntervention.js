...
export default function SelectTypeIntervention({
  selectedTypeIntervention,
  setSelectedTypeIntervention,
}) {
  ...
  const handleTypeInterventionChange = (value) => {
    const newTypeSelected = typeIntervention.find(
      (target) => parseID(target) === value,
    );
