...
export default function ShowIntervention() {
  ...
  const fetchIntervention = async () => {
    try {
      ...
      console.error(
        "Erreur lors de la récupération de l'intervention ou des utilisateurs",
        error,
      );
    } finally {
      setLoading(false);
  ...
  if (!intervention) {
    return (
      <Empty description="Intervention non trouvée" style={styles.empty} />
    );
  }

  const geoCoords = [
    client?.address?.coords.lng,
    client?.address?.coords.lat,
  ] || [0, 0];
          ...
          >
            <Descriptions bordered column={1}>
              ...
              <Descriptions.Item label="Durée">
                <DurationDisplay
                  seconds={intervention.typeIntervention?.duration}
                />
              </Descriptions.Item>
              <Descriptions.Item label="Prix Total Intervention">
                {intervention.typeIntervention?.price
                  ? `${intervention.typeIntervention?.price} €`
                  : "Non renseigné"}
              </Descriptions.Item>
