...
export default function ShowUser() {
  ...
  const userPosition = user?.address?.coords
    ? [user.address.coords.lat, user.address.coords.lng]
    : defaultPosition;
  ...
  if (!user) {
    return <Empty description="Utilisateur non trouvé" style={styles.empty} />;
  }
    ...
    <>
            ...
            <Descriptions bordered column={1}>
              ...
              <Descriptions.Item label="Adresse">
                {user.address.street +
                  ...
                  user.address.code +
                  " " +
                  user.address.city || "Non renseignée"}
              </Descriptions.Item>
        ...
        <Col span={11}>
          <MapContainer center={userPosition} zoom={13} style={styles.map}>
            ...
            <Marker position={userPosition} icon={markerIcon}>
              <Popup>{user.address?.value || "Adresse non renseignée"}</Popup>
            </Marker>
          </MapContainer>
        </Col>
      </Row>
      <Card>
        <Row style={styles.row}>
          ...
          <Button
            type="primary"
            onClick={() => nav(`/bikes/new/${parseID(user)}`)}
