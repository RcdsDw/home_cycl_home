 ...
 */
final class Version20250803182039 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        SQL);
        $this->addSql(<<<'SQL'
            CREATE TABLE zone (id UUID NOT NULL, name VARCHAR(150) NOT NULL, coords Geometry DEFAULT NULL, created_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updated_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY(id))
        SQL);
        $this->addSql(<<<'SQL'
            COMMENT ON COLUMN zone.id IS '(DC2Type:uuid)'
        SQL);
        $this->addSql(<<<'SQL'
            COMMENT ON COLUMN zone.coords IS '(DC2Type:geometry)'
        SQL);
        $this->addSql(<<<'SQL'
            ALTER TABLE zone ALTER COLUMN coords TYPE geometry(Polygon, 4326) USING ST_SetSRID(coords, 4326)
        SQL);
        $this->addSql(<<<'SQL'
